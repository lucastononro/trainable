...
async def compare(
    ...
    async with async_session() as db:
        ...
        for m in metrics_rows:
            per_metric[m.name][m.session_id].append(
                {"step": m.step, "value": m.value, "stage": m.stage}
            )
        out["metrics"] = {
            name: [
                {"session_id": sid, "points": pts} for sid, pts in by_session.items()
            ]
        ...
        # Cost totals
        usage_rows = (
            (await db.execute(select(UsageEvent).where(UsageEvent.session_id.in_(ids))))
            .scalars()
            .all()
        )
