"""Helpers for auxiliary tools that want a visible UI surface in the chat.

Distinct from `execute_code` (tool_start/tool_end with code+stdout) and from
`delegate_task` (subagent_start/subagent_end which represent a whole sub-agent
run). Use for tools whose effect is reading data (inspect_agent_context,
list_session_agents, read_project_session) or coordinating with other agents
(request_clarification when the parent answers directly).
...
rows and is queryable via `inspect_agent_context`. Friendly names are
...
async def emit_agent_tool_call(
    ...
    *,
    ...
    Single event (not start/end) because these tools are short-lived. If a
    tool needs progress indication later, add a separate `agent_tool_pending`
    event — don't conflate with this one.
    """
    payload = {
        "call_id": call_id,
        "tool_name": tool_name,
...
async def emit_clarification_exchange(
    ...
    *,
    ...
    directly" event. The user sees that an exchange happened but NOT the
    question or the answer — both are persisted under their respective
