...
# all of these is assumed current and only needs `alembic stamp head` (mark
# as up to date, no DDL) rather than `alembic upgrade head` (which would
# try — and fail — to (re)create tables that already exist). This assumption
...
def _run_alembic_sync(*, stamp_only: bool) -> None:
    """Runs Alembic's `upgrade head` (or `stamp head`) synchronously.
    ...
    if stamp_only:
        ...
        logger.info(
            ...
            " at head without altering schema"
        )
    else:
