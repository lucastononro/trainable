...
def _run_migrations(connection):
    """DEAD CODE — retained for history/rollback reference only, no longer
    called from `init_db`. See PR #121 (introduce Alembic migrations).

    This ~430-line hand-maintained sequence of guarded ALTER TABLE /
    destructive DELETE / backfill statements used to re-run on every boot
    with no versioning or rollback. It has been superseded by the Alembic
    revision chain in `alembic/versions/` — the initial revision was
    generated by autogenerating against `models.py` (which, by the time
    this was cut over, already reflected every change this function used
    to apply) and verified schema-equivalent against a DB built by
    `Base.metadata.create_all` + this function, on both SQLite and
    Postgres. The only discovered difference: this function creates two
    redundant duplicate indexes (`ix_dataset_versions_source_experiment`,
    `ix_registered_models_source_session` — note the missing `_id` suffix)
    that shadow the index SQLAlchemy already creates from the columns'
    `index=True` in `models.py`; the Alembic revision does not recreate
    that duplication.

    If Alembic ever needs to be rolled back, restore the call to this
    function from `init_db` (see git history) — do not delete it outright
    without checking git blame/PR #121 first.
    """
...
# Tables representative of the fully-migrated pre-Alembic schema. The
# initial Alembic revision (alembic/versions/0bae8e0f765d_initial_schema.py)
# was autogenerated against this exact end-state, so a DB that already has
# all of these is assumed current as of that revision and only needs
# `alembic stamp <initial>` (mark as up to date, no DDL) followed by
# `alembic upgrade head` for any post-cutover revisions, rather than a bare
# `alembic upgrade head` (which would try — and fail — to (re)create tables
# that already exist). This assumption
# holds for any DB that has been through a boot of the old `_run_migrations`
# path, since that function ran on every startup and brought the schema
# fully current before this cutover. A DB with only *some* of these tables
# (older than assumed, never fully migrated) isn't handled here and falls
# through to `upgrade head`, which will surface a loud DDL error rather than
# silently stamping an incomplete schema.
...
# The revision the legacy-schema equivalence above was verified against.
# Legacy DBs are stamped at THIS revision (not head) and then upgraded, so
# post-cutover revisions carrying real DDL (e.g. 3f7a1c9e2d64 adding
# projects.training_config, PR #164) still reach them.
_INITIAL_ALEMBIC_REVISION = "0bae8e0f765d"


def _pre_alembic_schema_present(connection) -> bool:
    """True if this DB already has the full current schema but hasn't been
    stamped with an Alembic version yet — see `_LEGACY_SCHEMA_MARKER_TABLES`."""
    insp = inspect(connection)
    if insp.has_table("alembic_version"):
        return False
    return all(insp.has_table(t) for t in _LEGACY_SCHEMA_MARKER_TABLES)
...
def _alembic_config():
    ...
    # Absolute, not the ini's literal "." — makes `env.py`'s `from config
    # import settings` / `from db import Base` / `import models` resolve
    # regardless of the caller's current working directory.
    cfg.set_main_option("prepend_sys_path", str(backend_dir))
    # env.py also reads settings.database_url directly, but setting it here
    # too keeps `alembic.config.Config` usable standalone (e.g. if some
    # future caller inspects cfg.get_main_option("sqlalchemy.url")).
    cfg.set_main_option("sqlalchemy.url", settings.database_url)
    return cfg


def _run_alembic_sync(*, stamp_only: bool) -> None:
    """Runs Alembic's `upgrade head` synchronously, stamping legacy DBs at
    the initial revision first.

    Must be called via `asyncio.to_thread` from async code: Alembic's
    env.py drives the app's *async* engine internally with its own
    `asyncio.run(...)` (see alembic/env.py), which raises if invoked from
    a thread that already has a running event loop. A fresh worker thread
    ...
    cfg = _alembic_config()
    if stamp_only:
        # Stamp at the INITIAL revision only — the legacy pre-Alembic schema
        # was verified equivalent to exactly that revision (PR #121), and
        # later revisions carry real DDL the legacy DB still needs. Stamping
        # straight at head would silently skip those.
        command.stamp(cfg, _INITIAL_ALEMBIC_REVISION)
        logger.info(
            "[DB] Existing pre-Alembic schema detected — stamped alembic_version"
            " at the initial revision without altering schema"
        )
    command.upgrade(cfg, "head")
    logger.info("[DB] Alembic migrations applied up to head")
...
async def init_db():
    ...
    async with engine.begin() as conn:
        stamp_only = await conn.run_sync(_pre_alembic_schema_present)
