...
class TestOpenAI:
    def test_codex_oauth_when_file_and_cli_present(self, monkeypatch):
        ...
        monkeypatch.setattr(openai_auth, "_codex_auth_file", lambda: openai_auth.Path("/home/u/.codex/auth.json"))
        monkeypatch.setattr(openai_auth.shutil, "which", lambda b: "/usr/bin/codex" if b == "codex" else None)
    ...
    def test_api_key_when_cli_missing_even_if_file_present(self, monkeypatch):
        from services.llm.auth import openai as openai_auth
        monkeypatch.setattr(openai_auth, "_codex_auth_file", lambda: openai_auth.Path("/somewhere/auth.json"))
        monkeypatch.setattr(openai_auth.shutil, "which", lambda b: None)
...
class TestGemini:
    def test_oauth_when_file_and_cli_present(self, monkeypatch):
        ...
        monkeypatch.setattr(gemini_auth, "_gemini_auth_file", lambda: gemini_auth.Path("/home/u/.gemini/oauth_creds.json"))
        monkeypatch.setattr(gemini_auth.shutil, "which", lambda b: "/usr/bin/gemini" if b == "gemini" else None)
