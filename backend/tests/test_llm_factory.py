...
def test_factory_failure_does_not_break_bootstrap(monkeypatch):
    """If one provider's import raises ImportError, the others still register."""
    from services.llm import factory
...
def test_bootstrap_does_not_swallow_non_import_errors(monkeypatch):
    """Regression for A5: pre-fix, every Exception was swallowed at debug
    level. A programming error (RuntimeError from a buggy top-level call,
    AttributeError from a typo, etc.) in a provider module would silently
    drop that provider — the user later hit "Unknown LLM provider 'claude'"
    with no hint why. After the narrow-except fix, only ImportError is
    suppressed; everything else propagates.
    """
    import sys
    import types
    ...
    # Build a stand-in `services.llm.claude_provider` whose attribute
    # lookup raises RuntimeError (NOT ImportError). `from x import y`
    # calls `getattr(x, 'y')` which dispatches into `__getattr__` and
    # propagates whatever it raises.
    bad_mod = types.ModuleType("services.llm.claude_provider")

    def _explode(_attr):
        raise RuntimeError("simulated programming error in provider module")
...
def test_litellm_default_model_is_claude_sonnet():
    """Regression for A6: the catch-all default should align with the
    project's primary provider (Claude Sonnet 4.6) rather than Groq's
    free tier."""
    from services.llm.litellm_provider import LiteLLMProvider

    assert LiteLLMProvider.capabilities.default_model == "anthropic/claude-sonnet-4-6"
